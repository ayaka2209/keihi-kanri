...
- **スコープ**: 経費・収入の記録、確定申告のための集計、固定資産の減価償却、
  そして**見積書の作成（取引先マスタ＋ブラウザ印刷でのPDF化）**まで。
  請求書・入金管理・レシートOCR・会計ソフト連携は将来の拡張（未着手）。
  ※ 見積書は青色申告の集計とは独立した受注業務の機能。詳細は
  [grasp-quotes](.claude/skills/grasp-quotes/SKILL.md)。
